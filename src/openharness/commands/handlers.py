...
from openharness.commands.types import CommandContext, CommandResult
...
def _coerce_setting_value(key: str, raw: str):
...
async def clear_handler(_: str, context: CommandContext) -> CommandResult:
...
async def context_handler(_: str, context: CommandContext) -> CommandResult:
...
async def usage_handler(_: str, context: CommandContext) -> CommandResult:
    usage = context.engine.total_usage
...
async def cost_handler(_: str, context: CommandContext) -> CommandResult:
    usage = context.engine.total_usage
...
async def stats_handler(_: str, context: CommandContext) -> CommandResult:
    settings = load_settings()
...
async def session_handler(args: str, context: CommandContext) -> CommandResult:
...
async def tag_handler(args: str, context: CommandContext) -> CommandResult:
    name = args.strip()
    if not name:
        return CommandResult(message="Usage: /tag NAME")
    return await session_handler(f"tag {name}", context)
...
async def agents_handler(args: str, context: CommandContext) -> CommandResult:
    tokens = args.split(maxsplit=1)
    guide = (
        "Subagent guide:\n"
...
async def init_handler(args: str, context: CommandContext) -> CommandResult:
    del args
...
async def onboarding_handler(_: str, context: CommandContext) -> CommandResult:
    del context
    return CommandResult(
        message=(
...
async def provider_handler(args: str, context: CommandContext) -> CommandResult:
    manager = AuthManager()
...
async def theme_handler(args: str, context: CommandContext) -> CommandResult:
...
async def keybindings_handler(_: str, context: CommandContext) -> CommandResult:
...
async def vim_handler(args: str, context: CommandContext) -> CommandResult:
    settings = load_settings()
    current = (
...
async def voice_handler(args: str, context: CommandContext) -> CommandResult:
...
async def rate_limit_options_handler(_: str, context: CommandContext) -> CommandResult:
    settings = load_settings()
...
async def release_notes_handler(_: str, context: CommandContext) -> CommandResult:
...
async def upgrade_handler(_: str, context: CommandContext) -> CommandResult:
    del context
    try:
        version = importlib.metadata.version("openharness")
    except importlib.metadata.PackageNotFoundError:
...
async def tasks_handler(args: str, context: CommandContext) -> CommandResult:
...
async def autopilot_handler(args: str, context: CommandContext) -> CommandResult:
...
async def ship_handler(args: str, context: CommandContext) -> CommandResult:
    raw = args.strip()
    if not raw:
