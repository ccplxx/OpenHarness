...
def build_dry_run_preview(
    *,
    ...
    preview = {
        ...
        "prompt_preview": safe_short(preview_prompt or "", limit=220) if preview_prompt else "",
        "settings": {
        ...
        "system_prompt_preview": safe_short(system_prompt_text, limit=600),
    }
...
def format_dry_run_preview(preview: dict[str, object]) -> str:
    ...
    if skills:
        ...
        for entry in skills[:8]:
            lines.append(f"- {entry.get('name')}: {safe_short(str(entry.get('description') or ''), limit=100)}")
        if len(skills) > 8:
